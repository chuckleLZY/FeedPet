<template>
  <div>
    ...
    <el-card
      class="resumeCard"
      v-if="editResume == false && resumeExsit == true"
      v-loading="loading"
    >
      <el-row class="header">
      ...
      <el-row>
        <el-col :span="24">
          <p class="name">
            <i class="el-icon-user-solid"></i> {{ Resume.student_name }}
          </p>
          <p class="resumeInfo" v-if="UserInfo.gender == true">男</p>
          <p class="resumeInfo" v-if="UserInfo.gender == false">女</p>
          <p class="resumeInfo">
            <span> {{ Resume.age }} </span> 岁
          </p>
          <p class="resumeInfo">
            <span style="font-family:'楷体';"> {{ Resume.city }} </span>
          </p>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="24">
          <p class="tips">
            ...
            为保证信息真实性，以下信息与您的个人信息绑定，如需修改请修改个人信息
          </p>
        </el-col>
      </el-row>
      <div>
    ...
    <el-card class="editCard" v-if="editResume == true && resumeExsit == true">
      ...
      >
        <el-form-item label="姓名:" style="width:40%" prop="student_name">
          <el-input
            placeholder="请输入姓名"
            v-model="addResume.student_name"
            maxlength="5"
            show-word-limit
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="主要技能:" prop="skill">
          <el-input
            placeholder="请输入您掌握的主要技能"
            v-model="addResume.skill"
            type="textarea"
            :rows="5"
            maxlength="300"
            show-word-limit
        ...
        <el-form-item label="学术成就:" prop="academic">
          <el-input
            placeholder="请输入您取得的学术成就"
            v-model="addResume.academic"
            type="textarea"
            :rows="5"
            maxlength="300"
            show-word-limit
        ...
        <el-form-item label="项目经历:" prop="project">
          <el-input
            placeholder="请输入您参加的项目经历"
            v-model="addResume.project"
            type="textarea"
            :rows="5"
            maxlength="300"
            show-word-limit
        ...
        <el-form-item label="社团/活动经历:" prop="project">
          <el-input
            placeholder="请输入您的社团经历或者参与活动经历"
            v-model="addResume.community"
            type="textarea"
            :rows="5"
            maxlength="300"
            show-word-limit
        ...
        <el-form-item label="个人简介:" prop="introduction">
          <el-input
            placeholder="在这里简单地介绍您自己"
            v-model="addResume.introduction"
            type="textarea"
            :rows="6"
            maxlength="400"
            show-word-limit
...
export default {
  data() {
    return {
      ...
      addResumeRules: {
        student_name: [{ required: true, message: "请输入您的姓名" }],
        education: [{ required: true, message: "请输入教育经历" }],
        skill: [{ required: true, message: "请输入主要技能" }],
        academic: [{ required: true, message: "请输入学术成就" }],
        project: [{ required: true, message: "请输入项目经历" }],
        community: [{ required: true, message: "请输入社团/活动经历" }],
  ...
  methods: {
    ...
    async getResume() {
      const { data: res } = await axios.post(
        this.$helper.endpointUrl("/Resume/GetResume"),
        {},
        { withCredentials: true }
      );
    ...
    save() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        const { data: res } = await axios.put(
          this.$helper.endpointUrl("/Resume/UpdateResume"),
          this.addResume,
          { withCredentials: true }
        );
        if (res.status > 400) {
    ...
    createResume() {
      this.$refs.createFormRef.validate(async (valid) => {
        if (!valid) {
          this.$message.error(
            "只有按照验证提示完成所有简历信息的正确填写后才能创建简历哦"
          );
          return;
        }
        const { data: res } = await axios.post(
          this.$helper.endpointUrl("/Resume/CreateResume"),
          this.addResume,
          { withCredentials: true }
        );
        if (res.status > 400) {
